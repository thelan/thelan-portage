...
IUSE="jpeg new-login opengl pam suid xinerama gnome"
...
src_install() {
	...
	# thelan gnome patch
	if [[ "$(use_with gnome)" = "--with-gnome" ]]
	then
	  ln -snf /usr/bin/xscreensaver /usr/bin/gnome-screensaver
	  ln -snf /usr/bin/xscreensaver-command /usr/bin/gnome-screensaver-command
	fi
}
